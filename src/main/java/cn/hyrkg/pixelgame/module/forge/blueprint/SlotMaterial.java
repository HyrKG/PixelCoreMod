...
public class SlotMaterial extends CustomSlot {

	private GuiBlueprint gui;
	private String text = "";// 数量（需求）文本

	public SlotMaterial(GuiBlueprint gui, String text, Slot slot, int id, int x, int y, int size, double scale) {
	...
	public void drawItem(Minecraft mc, RenderItem itemRender, ItemStack item, String s) {
		...
		if (bindingSlot.getHasStack()) {
			...
			mc.renderEngine.bindTexture(GuiBlueprint.TEX_BLUEPRINT.getTexture());
			mc.ingameGUI.drawModalRectWithCustomSizedTexture(0, 0, 371, 23, 16, 15, GuiBlueprint.TEX_BLUEPRINT.texWidth,
					GuiBlueprint.TEX_BLUEPRINT.texHeight);
			GL11.glPopMatrix();
			...
			mc.ingameGUI.drawModalRectWithCustomSizedTexture(6, 4, 139, 240, 9, 11, GuiBlueprint.TEX_BLUEPRINT.texWidth,
					GuiBlueprint.TEX_BLUEPRINT.texHeight);
			GL11.glPopMatrix();
		}
