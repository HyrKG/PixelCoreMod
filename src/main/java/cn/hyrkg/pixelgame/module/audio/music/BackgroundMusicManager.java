...
/**
 * This manager allows the manage the music on client-side. (Load resource, stop
 * music, etc...)
 * 
 * @author Yuti
 *
...
public class BackgroundMusicManager {
	...
	/**
	 * Manages the playlist of the BGM through the
	 * {@link BackgroundMusicManager#player}.
	 */
	...
	/**
	 * A map listing all the sounds currently playing on the client (obtained
	 * through reflection)
	 */
...
//		AudioSourceManagers.registerRemoteSources(manager);
		...
		SoundManager soundManager = (SoundManager) ReflectionHelper.getPrivateValue(SoundHandler.class,
				this.soundHandler, "sndManager", "field_147694_f");
		this.soundSystem = (SoundSystem) ReflectionHelper.getPrivateValue(SoundManager.class, soundManager, "sndSystem",
				"field_148620_e");
		this.minecraftPlayingSounds = (Map<String, ISound>) ReflectionHelper.getPrivateValue(SoundManager.class,
				soundManager, "playingSounds", "field_148629_h");
	...
	/**
	 * Update the current music volume according to the volume specified in the
	 * config and global Minecraft's master volume.
	 * 
	 * @param volume
	 */
	private void updateMusicVolume(int volume) {
		this.player.setVolume(Math.min(volume, this.getMinecraftBindingVolume()));
	 ...
	 */
	public int getMinecraftBindingVolume() {
		float musicLevel = Minecraft.getMinecraft().gameSettings.getSoundLevel(SoundCategory.MUSIC);
		return (int) (musicLevel * 100);
	}

	/**
	 * Send a {@link PacketAskRefresh} packet to the server to ask a refresh of the
	 * BGM. It will mainly be used when a client reactivate the mod after disabling
	 * it, to keep it updated with the current region's BGM.
	 */
